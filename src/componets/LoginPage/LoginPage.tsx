...
const LoginPage: React.FC = () => {
  ...
  const [showAlert, setShowAlert] = useState<setAlertShowType>({ show: false, msgInfo: '', severity: "error" })
  const dispatch = useDispatch()

  const signin = (newtoken: string, id_user: string) => {
  ...
  const auth_user = () => {
    ...
    get_jwt(email.trim().toLowerCase(), password.trim())
      .then(function (res) {
  ...
  const passwordChange = (e: { target: { value: React.SetStateAction<string>; } }) => {
        ...
        <Grid md={12} display="flex" justifyContent="center" alignItems="center">
          {showConfirmMail ? <ConfirmMail setShowConfirmMail={setShowConfirmMail} email={email} idUser={idUser} /> :
            ...
            >
              ...
              <AlertInfo showAlert={showAlert.show} setAlertShow={setShowAlert} severity={showAlert.severity} value={showAlert.msgInfo || ''} />
